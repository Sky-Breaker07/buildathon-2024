<template>
  ...
  >
    ...
    >
      <template #count>
        <CountUp :end-val="staffStats.counts.superAdmins" :duration="2" />
      </template>
    </DashboardCard>
    ...
    <DashboardCard
      ...
      :subtitle="`${
        ...
        staffStats.counts.healthCareProfessionals.admins !== 1 ? 's' : ''
      }`"
      colorScheme="blue"
    ...
    >
      <template #count>
        <CountUp :end-val="professionData.count" :duration="2" />
      </template>
    </DashboardCard>
  ...
  <InfoModal v-if="showModal" @close="closeModal">
    <template #header>
      <h3 class="text-lg font-semibold">{{ formatRole(selectedRole) }}</h3>
    </template>
    <template #body>
      <div class="max-h-96 overflow-y-auto">
        <ul>
          <li v-for="staff in getStaffList()" :key="staff.staff_id">
            ...
            <span v-if="staff.profession" class="text-gray-500 ml-2"
              >({{ staff.profession }})</span
            >
            <ul v-if="staff.subordinates" class="ml-4">
              <li v-for="sub in staff.subordinates" :key="sub.staff_id">
                {{ sub.name }} ({{ sub.staff_id }})
                <span v-if="sub.profession" class="text-gray-500 ml-2"
                  >({{ sub.profession }})</span
                >
              </li>
            </ul>
...
const formatRole = (role) => {
  ...
  if (category === "healthCareProfessionals" && subCategory !== "admins") {
    return subCategory;
  } else {
    return role
      .split(".")
