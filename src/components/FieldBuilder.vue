	...
	<div class="field-builder space-y-4 p-4 bg-white rounded-lg shadow-md">
		...
		<div class="relative">
			<input
				...
				:placeholder="
					sectionName ? `${sectionName} variable` : 'Variable'
				"
				required
			/>
		...
		>
			...
			<p class="text-xs text-gray-500">
				This field will allow image uploads up to 5MB during assessment.
			</p>
		</div>

		<div
			class="relative"
			v-if="field.type !== 'Boolean' && !field.isImage"
		>
			<input
		...
		>
			...
			<button
				@click="addOption"
				class="mt-1 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors duration-300 flex items-center"
			>
				<v-icon
					name="ri-add-line"
					class="mr-2"
...
<script setup>
	...
	const addOption = () => {
		const newOptions = Array.isArray(props.field.options)
			? [...props.field.options, '']
			: [''];
