...
<template>
  <div class="container mx-auto px-4 py-8">
    <h1 class="text-4xl font-bold mb-8 text-center text-indigo-800">Patient Statistics Dashboard</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <DynamicStatistics
        ...
        statType="getPatientsByBiodata"
        :params="{ field: 'sex', operator: 'in', value: ['male', 'female'] }"
        chartType="bar"
        icon="ri-men-line"
      />
      ...
      <DynamicStatistics
        ...
        statType="getPatientsByBiodata"
        :params="{ field: 'occupation', operator: 'ne', value: 'Student' }"
        chartType="pie"
    ...
    <div class="mt-12 bg-white shadow-lg rounded-lg p-6">
      <h2 class="text-2xl font-bold mb-4 text-indigo-800">Custom Query</h2>
      <div class="flex flex-wrap gap-4 mb-4">
        ...
        <select v-model="customQuery.operator" class="p-2 border rounded focus:ring-2 focus:ring-indigo-500">
          <option
        ...
        <div v-else class="flex flex-col">
          ...
          >
            <v-icon name="ri-add-line" class="mr-1" /> Add Value
          </button>
        </div>
        <button
        ...
        >
          <v-icon name="ri-search-line" class="mr-1" /> Execute Query
        </button>
      </div>
      <DynamicStatistics
...
const currentFieldOptions = computed(() => {
  if (
    ...
    return getNestedFields(queryableFields.value[customQuery.value.model]);
  }
  return [];
});
...
const getFieldType = (model, field) => {
  if (model && field) {
    const nestedFields = getNestedFields(queryableFields.value[model], true);
...
const getNestedFields = (obj, isFlat = false, prefix = "") => {
  ...
  for (const [key, value] of Object.entries(obj)) {
    if (typeof value === "object" && value !== null && !Array.isArray(value)) {
      const nestedFields = getNestedFields(value, isFlat, `${prefix}${key}.`);
      if (isFlat) {
      ...
      if (isFlat) {
        fields[`${prefix}${key}`] = value === "Unknown" ? "String" : value;
      } else {
...
const getStatType = () => {
  ...
  const modelName =
    customQuery.value.model === "BioData" ? "Biodata" : customQuery.value.model;
...
const getCustomQueryParams = () => {
  return {
    ...
    value: ['in', 'nin'].includes(customQuery.value.operator) ? customQuery.value.values : customQuery.value.value,
  };
};
