<template>
    <div class="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div class="max-w-7xl mx-auto">
        ...
        <div v-if="activeTab === 'new' && patientData">
          <div v-if="evaluationTemplates.length === 0" class="bg-white shadow-lg rounded-lg overflow-hidden p-6">
            <p class="text-lg text-gray-600">No evaluation templates are available for your profession.</p>
            <p class="text-md text-gray-500 mt-2">Please contact an administrator to create templates before adding new evaluations.</p>
          </div>
          <!-- Template Selection -->
          <div
          ...
          >
            ...
            <div class="p-6">
              <div v-if="evaluationTemplates.length === 0" class="text-center">
                <p class="text-lg text-gray-600">No evaluation templates available for your profession.</p>
                ...
                >
                  ...
                  >
                    ...
                    <component
                      ...
                      @input="updateEvaluationData(sectionName, fieldKey, $event.target.value)"
        ...
        <div v-else-if="activeTab === 'past' && patientData" class="space-y-8">
          <div v-if="Object.keys(groupedEvaluations).length === 0" class="bg-white shadow-lg rounded-lg overflow-hidden p-6">
            <p class="text-lg text-gray-600">No past evaluations found for this patient.</p>
          </div>
          <div
            v-else
  ...
  * {
  ...
  const groupedEvaluations = computed(() => {
    if (!patientData.value || !patientData.value.evaluations || patientData.value.evaluations.length === 0) return {};
  ...
  const fetchEvaluationTemplates = async () => {
    try {
      ...
      if (error.response && error.response.status === 404) {
        toast.warning("No evaluation templates found for your profession. Please contact an administrator.");
  ...
  const selectTemplate = (template) => {
    ...
    evaluationData.value = Object.keys(template.fields).reduce((acc, sectionName) => {
    ...
    console.log('Initial Evaluation Data:', evaluationData.value); // For debugging
  };
  
  const resetForm = () => {
    if (selectedTemplate.value) {
      evaluationData.value = Object.keys(selectedTemplate.value.fields).reduce((acc, sectionName) => {
  ...
  const submitEvaluation = async () => {
    try {
      ...
      const transformedEvaluationData = Object.entries(evaluationData.value).reduce((acc, [sectionName, sectionData]) => {
