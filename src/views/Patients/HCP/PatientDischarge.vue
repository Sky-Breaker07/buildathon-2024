<template>
    <div class="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div class="max-w-7xl mx-auto">
        ...
        <div v-if="activeTab === 'new' && patientData">
          <div v-if="dischargeTemplates.length === 0" class="bg-white shadow-lg rounded-lg overflow-hidden p-6">
            <p class="text-lg text-gray-600">No discharge templates are available for your profession.</p>
            <p class="text-md text-gray-500 mt-2">Please contact an administrator to create templates before adding new discharges.</p>
          </div>
          <!-- Template Selection -->
          <div
          ...
          >
            ...
            <div class="p-6">
              <div v-if="dischargeTemplates.length === 0" class="text-center">
                <p class="text-lg text-gray-600">No discharge templates available for your profession.</p>
                ...
                >
                  ...
                  >
                    ...
                    <component
                      ...
                      @input="updateDischargeData(sectionName, fieldKey, $event.target.value)"
        ...
        <div v-else-if="activeTab === 'past' && patientData" class="space-y-8">
          <div v-if="Object.keys(groupedDischarges).length === 0" class="bg-white shadow-lg rounded-lg overflow-hidden p-6">
            <p class="text-lg text-gray-600">No past discharges found for this patient.</p>
          </div>
          <div
            v-else
            ...
            <div class="p-6">
              ...
              >
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                  <div
                    v-for="(sectionData, sectionName) in discharge.discharge_data"
                    :key="sectionName"
                    class="mb-4"
                  >
  ...
  * {
  ...
  const groupedDischarges = computed(() => {
    if (!patientData.value || !patientData.value.discharges || patientData.value.discharges.length === 0) return {};
  ...
  const fetchDischargeTemplates = async () => {
    try {
      loadingModal.value.show();
      const response = await getDischargeTemplatesByProfession(
      ...
      if (error.response && error.response.status === 404) {
        toast.warning("No discharge templates found for your profession. Please contact an administrator.");
  ...
  const selectTemplate = (template) => {
    ...
    dischargeData.value = Object.keys(template.fields).reduce((acc, sectionName) => {
    ...
    console.log('Initial Discharge Data:', dischargeData.value); // For debugging
  };
  
  const resetForm = () => {
    if (selectedTemplate.value) {
      dischargeData.value = Object.keys(selectedTemplate.value.fields).reduce((acc, sectionName) => {
  ...
  const submitDischarge = async () => {
    try {
      ...
      // Transform the dischargeData to use labels instead of field keys
      const transformedDischargeData = Object.entries(dischargeData.value).reduce((acc, [sectionName, sectionData]) => {
      ...
      console.log('Transformed Discharge Data:', transformedDischargeData); // For debugging
      ...
      if (response.data && response.data.status === "Success") {
        ...
        resetForm();
        await fetchPatientData(); // Refresh patient data to include the new discharge
        activeTab.value = 'past'; // Switch to past discharges tab
      } else {
