...
<template>
  <div class="min-h-screen px-4 sm:px-6 lg:px-8">
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <!-- Tabs for New Discharge and Past Discharges -->
      <div v-if="patientData" class="mb-8">
        <div class="sm:hidden">
          ...
          >
            <option value="new">New Discharge</option>
            <option value="past">Past Discharges</option>
          </select>
        </div>
      ...
      <div v-if="activeTab === 'new' && patientData">
        <div
          v-if="dischargeTemplates.length === 0"
          class="bg-white shadow-lg rounded-lg overflow-hidden p-6"
        >
          <p class="text-lg text-gray-600">
            No discharge templates are available for your profession.
          </p>
        ...
        >
          <div class="px-6 py-4 bg-gradient-to-r from-green-500 to-teal-600">
            <h2 class="text-2xl font-bold text-white flex items-center">
              ...
              Select Discharge Template
            </h2>
          </div>
          <div class="p-6">
            <div v-if="dischargeTemplates.length === 0" class="text-center">
              <p class="text-lg text-gray-600">
                No discharge templates available for your profession.
              </p>
              <p class="text-md text-gray-500 mt-2">
            ...
            >
              <div
                v-for="template in dischargeTemplates"
                :key="template._id"
        ...
        >
          ...
          <div class="p-6">
            <form @submit.prevent="submitDischarge" class="space-y-8">
              <div
                ...
                >
                  ...
                  <template v-if="field.isImage">
                    <div class="flex items-center space-x-4">
                      ...
                      <span
                        v-if="dischargeData[sectionName][fieldKey]"
                    ...
                    >
                      <img
                        :src="dischargeData[sectionName][fieldKey]"
                        alt="Uploaded image"
                  ...
                  <component
                    ...
                    :id="`${sectionName}_${fieldKey}`"
                    :value="dischargeData[sectionName][fieldKey]"
                    @input="
                      updateDischargeData(
                        sectionName,
                        fieldKey,
      ...
      <div v-else-if="activeTab === 'past' && patientData" class="space-y-8">
        <div
          v-if="Object.keys(groupedDischarges).length === 0"
          class="bg-white shadow-lg rounded-lg overflow-hidden p-6"
        >
          <p class="text-lg text-gray-600">
            No past discharges found for this patient.
          </p>
        </div>
        <div v-else>
          <div
            v-for="(discharges, profession) in groupedDischarges"
            :key="profession"
            class="bg-white shadow-lg rounded-lg overflow-hidden mb-6 transition-all duration-300 ease-in-out hover:shadow-xl"
          >
            <div
              @click="toggleAccordion(profession)"
              class="px-6 py-4 bg-gradient-to-r from-purple-500 to-pink-600 cursor-pointer flex justify-between items-center transition-all duration-300 ease-in-out hover:from-purple-600 hover:to-pink-700"
            >
              <h2 class="text-2xl font-bold text-white">
                {{ profession }} Discharges
              </h2>
              <v-icon
                :name="
                  expandedProfessions[profession]
                    ? 'ri-arrow-up-s-line'
                    : 'ri-arrow-down-s-line'
                "
                class="text-white text-2xl transition-transform duration-300"
                :class="{
                  'transform rotate-180': expandedProfessions[profession],
                }"
              />
            </div>
            <transition
            ...
            >
              <div
                v-show="expandedProfessions[profession]"
                class="overflow-hidden"
              >
                <div class="p-6 space-y-6">
                  <div
                    v-for="(discharge, index) in discharges"
                    :key="index"
                    class="bg-white shadow-md rounded-lg overflow-hidden transition-all duration-300 ease-in-out hover:shadow-lg"
                  >
                    <div
                      class="px-6 py-4 bg-gradient-to-r from-purple-500 to-pink-600 text-white"
                    >
                      <h3 class="text-xl font-semibold">
                        {{ discharge.template?.name || "Unnamed Discharge" }}
                      </h3>
                    ...
                    <div class="p-6 space-y-6">
                      ...
                      >
                        ...
                        >
                          ...
                          <p v-else class="text-base text-gray-900">
                            {{ value }}
                          </p>
                        </div>
                      </div>
    ...
    >
      <div class="relative w-full h-full p-4 flex items-center justify-center">
        ...
        <button
          ...
          class="absolute top-4 right-4 text-white text-4xl hover:text-gray-300 transition-colors duration-300 focus:outline-none"
        >
          &times;
        </button>
        <button
          @click="zoomIn"
          class="absolute bottom-4 right-20 bg-white text-gray-800 rounded-full p-2 hover:bg-gray-200 transition-colors duration-300 focus:outline-none"
        >
          <v-icon name="ri-zoom-in-line" scale="1.5" />
        </button>
        <button
          @click="zoomOut"
          class="absolute bottom-4 right-4 bg-white text-gray-800 rounded-full p-2 hover:bg-gray-200 transition-colors duration-300 focus:outline-none"
        >
          <v-icon name="ri-zoom-out-line" scale="1.5" />
        </button>
      </div>
    </div>
  </div>
...
const toggleAccordion = (profession) => {
  expandedProfessions.value[profession] =
    !expandedProfessions.value[profession];
};
...
const updateDischargeData = (sectionName, fieldKey, value) => {
  if (selectedTemplate.value.fields[sectionName][fieldKey].type === "Boolean") {
    dischargeData.value[sectionName][fieldKey] = value === "true";
  } else {
    dischargeData.value[sectionName][fieldKey] = value;
  }
};
...
const groupedDischarges = computed(() => {
  if (
    !patientData.value ||
    !patientData.value.discharges ||
    patientData.value.discharges.length === 0
  )
    return {};
  return patientData.value.discharges.reduce((acc, discharge) => {
    const profession = discharge.template?.profession || "Unknown";
    if (!acc[profession]) acc[profession] = [];
    acc[profession].push(discharge);
    return acc;
  }, {});
});
...
const fetchDischargeTemplates = async () => {
  try {
    ...
    dischargeTemplates.value = response.data.dischargeTemplates;
  } catch (error) {
    console.error("Error fetching discharge templates:", error);
    if (error.response && error.response.status === 404) {
      toast.warning(
        "No discharge templates found for your profession. Please contact an administrator."
      );
      dischargeTemplates.value = [];
    } else {
      toast.error("Failed to fetch discharge templates. Please try again.");
    }
  } finally {
    loadingModal.value.hide();
  }
...
const fetchPatientData = async () => {
  try {
    ...
    if (
      ...
      "Fetched patient data:", patientData.value; // For debugging
    } else {
...
const selectTemplate = (template) => {
  ...
  dischargeData.value = Object.keys(template.fields).reduce(
    (acc, sectionName) => {
  ...
  "Initial Discharge Data:", dischargeData.value; // For debugging
};

const resetForm = () => {
  if (selectedTemplate.value) {
    dischargeData.value = Object.keys(selectedTemplate.value.fields).reduce(
      (acc, sectionName) => {
...
const handleImageUpload = async (event, sectionName, fieldKey) => {
  ...
  loadingModal.value.show();
  try {
    const downloadURL = await uploadImage(file, "Discharges");
    if (downloadURL) {
      updateDischargeData(sectionName, fieldKey, downloadURL);
...
const deleteImage = async (sectionName, fieldKey) => {
  const imageUrl = dischargeData.value[sectionName][fieldKey];
  if (!imageUrl) return;

  loadingModal.value.show();
  try {
    await deleteFirebaseImage(imageUrl);
    updateDischargeData(sectionName, fieldKey, "");
...
const submitDischarge = async () => {
  try {
    ...
    const transformedDischargeData = Object.entries(dischargeData.value).reduce(
    ...
    "Transformed Discharge Data:", transformedDischargeData;

    const response = await createDischarge(
      selectedTemplate.value._id,
      transformedDischargeData,
      patientData.value.hospital_record.hospital_id
    );

    if (response.data && response.data.status === "Success") {
      toast.success("Discharge submitted successfully!");
      resetForm();
      await fetchPatientData();
    ...
    toast.error(`Failed to submit discharge: ${error.message}`);
  } finally {
    loadingModal.value.hide();
  }
};
...
const openFullScreenImage = (imageUrl) => {
  ...
  imageZoom.value = 1; // Reset zoom when opening new image
};

const closeFullScreenImage = () => {
  fullScreenImage.value = null;
  imageZoom.value = 1; // Reset zoom when closing
};
