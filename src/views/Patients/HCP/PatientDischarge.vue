<template>
	<div class="min-h-screen px-4 sm:px-6 lg:px-8">
		...
		<div class="max-w-7xl mx-auto">
			<h1
				class="text-4xl font-extrabold text-gray-900 mb-8 flex items-center"
			>
				<v-icon
			...
			>
				<div
					class="px-6 py-4 bg-gradient-to-r from-blue-500 to-indigo-600"
				>
			...
			<div v-if="activeTab === 'new' && patientData">
				...
				>
					<p class="text-lg text-gray-600">
						No discharge templates are available for your
						profession.
					</p>
					<p class="text-md text-gray-500 mt-2">
						Please contact an administrator to create templates
						before adding new discharges.
					</p>
				</div>
				<!-- Template Selection -->
				<div
				...
				>
					<div
						class="px-6 py-4 bg-gradient-to-r from-green-500 to-teal-600"
					>
						<h2
					...
					<div class="p-6">
						...
						>
							<p class="text-lg text-gray-600">
								No discharge templates available for your
								profession.
							</p>
							<p class="text-md text-gray-500 mt-2">
				...
				>
					...
					<div class="p-6">
						<form
							@submit.prevent="submitDischarge"
							class="space-y-8"
						>
							<div
								v-for="(
									sectionFields, sectionName
								) in selectedTemplate.fields"
								:key="sectionName"
								class="space-y-6"
							>
								...
								>
									...
									<component
										...
										:id="`${sectionName}_${fieldKey}`"
										:value="
											dischargeData[sectionName][fieldKey]
										"
										@input="
											updateDischargeData(
												sectionName,
			...
			<!-- Past Discharges -->
			<div
				v-else-if="activeTab === 'past' && patientData"
				class="space-y-8"
			>
				<div
								...
								<div class="p-6">
									...
									>
										...
										<div class="p-6">
											...
											>
												<h4
													class="text-lg font-medium text-gray-800 mb-3 border-b border-gray-200 pb-2"
												>
													{{ sectionName }}
												</h4>
												<div
...
<script setup>
	...
	const updateDischargeData = (sectionName, fieldKey, value) => {
		if (
			selectedTemplate.value.fields[sectionName][fieldKey].type ===
			'Boolean'
		) {
	...
	const fetchDischargeTemplates = async () => {
		try {
			...
			if (error.response && error.response.status === 404) {
				...
				toast.error(
					'Failed to fetch discharge templates. Please try again.'
				);
			}
		} finally {
			loadingModal.value.hide();
	...
	const resetForm = () => {
		if (selectedTemplate.value) {
			dischargeData.value = Object.keys(
				...
				acc[sectionName] = Object.keys(
					...
					sectionAcc[fieldKey] =
						selectedTemplate.value.fields[sectionName][fieldKey]
							.defaultValue || '';
					return sectionAcc;
				}, {});
				return acc;
	...
	const submitDischarge = async () => {
		try {
			...
			const transformedDischargeData = Object.entries(
				dischargeData.value
