<template>
    ...
    <div class="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div class="max-w-7xl mx-auto">
        ...
        <div v-if="activeTab === 'new' && patientData">
          <div v-if="treatmentTemplates.length === 0" class="bg-white shadow-lg rounded-lg overflow-hidden p-6">
            <p class="text-lg text-gray-600">No treatment templates are available for your profession.</p>
            <p class="text-md text-gray-500 mt-2">Please contact an administrator to create templates before adding new treatments.</p>
          </div>
          <!-- Template Selection -->
          <div
          ...
          >
            ...
            <div class="p-6">
              <div v-if="treatmentTemplates.length === 0" class="text-center">
                <p class="text-lg text-gray-600">No treatment templates available for your profession.</p>
                ...
                >
                  ...
                  >
                    ...
                    <component
                      ...
                      @input="updateTreatmentData(sectionName, fieldKey, $event.target.value)"
        ...
        <div v-else-if="activeTab === 'past' && patientData" class="space-y-8">
          <div v-if="Object.keys(groupedTreatments).length === 0" class="bg-white shadow-lg rounded-lg overflow-hidden p-6">
            <p class="text-lg text-gray-600">No past treatments found for this patient.</p>
          </div>
          <div
            v-else
            ...
            <div class="p-6">
              ...
              >
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                  <div
                    v-for="(sectionData, sectionName) in treatment.treatment_data"
                    :key="sectionName"
                    class="mb-4"
                  >
  ...
  * {
  ...
  import { getTreatmentTemplatesByProfession } from "@/utils/treatmentTemplate";
  ...
  const groupedTreatments = computed(() => {
    if (!patientData.value || !patientData.value.treatments || patientData.value.treatments.length === 0) return {};
  ...
  const fetchTreatmentTemplates = async () => {
    try {
      loadingModal.value.show();
      const response = await getTreatmentTemplatesByProfession(
      ...
      if (error.response && error.response.status === 404) {
        toast.warning("No treatment templates found for your profession. Please contact an administrator.");
  ...
  const selectTemplate = (template) => {
    ...
    treatmentData.value = Object.keys(template.fields).reduce((acc, sectionName) => {
    ...
    console.log('Initial Treatment Data:', treatmentData.value); // For debugging
  };
  
  const resetForm = () => {
    if (selectedTemplate.value) {
      treatmentData.value = Object.keys(selectedTemplate.value.fields).reduce((acc, sectionName) => {
  ...
  const submitTreatment = async () => {
    try {
      ...
      // Transform the treatmentData to use labels instead of field keys
      const transformedTreatmentData = Object.entries(treatmentData.value).reduce((acc, [sectionName, sectionData]) => {
      ...
      console.log('Transformed Treatment Data:', transformedTreatmentData); // For debugging
      ...
      if (response.data && response.data.status === "Success") {
        ...
        resetForm();
        await fetchPatientData(); // Refresh patient data to include the new treatment
        activeTab.value = 'past'; // Switch to past treatments tab
      } else {
