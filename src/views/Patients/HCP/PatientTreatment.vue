<template>
	<div class="min-h-screen px-4 sm:px-6 lg:px-8">
	  ...
	  <div class="max-w-7xl mx-auto">
		...
		<!-- Tabs for New Treatment and Past Treatments -->
		<div v-if="patientData" class="mb-8">
		  <div class="sm:hidden">
			...
			>
			  ...
			  <option value="past">Past Treatments</option>
			</select>
		  </div>
		...
		<div v-if="activeTab === 'new' && patientData">
		  <div
			v-if="treatmentTemplates.length === 0"
			class="bg-white shadow-lg rounded-lg overflow-hidden p-6"
		  >
			<p class="text-lg text-gray-600">
			  No treatment templates are available for your profession.
			</p>
		  ...
		  >
			...
			<div class="p-6">
			  <div v-if="treatmentTemplates.length === 0" class="text-center">
				<p class="text-lg text-gray-600">
				  No treatment templates available for your profession.
				</p>
				<p class="text-md text-gray-500 mt-2">
			  ...
			  >
				<div
				  v-for="template in treatmentTemplates"
				  :key="template._id"
		  ...
		  >
			...
			<div class="p-6">
			  <form @submit.prevent="submitTreatment" class="space-y-8">
				<div
				  ...
				  >
					...
					<template v-if="field.isImage">
					  <div class="flex items-center space-x-4">
						...
						<span
						  v-if="treatmentData[sectionName][fieldKey]"
					  ...
					  >
						<img
						  :src="treatmentData[sectionName][fieldKey]"
						  alt="Uploaded image"
					...
					<component
					  ...
					  :id="`${sectionName}_${fieldKey}`"
					  :value="treatmentData[sectionName][fieldKey]"
					  @input="
						updateTreatmentData(
						  sectionName,
						  fieldKey,
		...
		<div v-else-if="activeTab === 'past' && patientData" class="space-y-8">
		  <div
			v-if="Object.keys(groupedTreatments).length === 0"
			class="bg-white shadow-lg rounded-lg overflow-hidden p-6"
		  >
			<p class="text-lg text-gray-600">
			  No past treatments found for this patient.
			</p>
		  </div>
		  <div v-else>
			<div
			  v-for="(treatments, profession) in groupedTreatments"
			  :key="profession"
					...
					>
					  ...
					  >
						<h3 class="text-xl font-semibold">
						  {{ treatment.template?.name || "Unnamed Treatment" }}
						</h3>
						<p class="text-sm text-indigo-100">
						  {{ new Date(treatment.createdAt).toLocaleString() }}
						</p>
					  </div>
					  <div class="p-6 space-y-6">
						<div
  ...
  import { createTreatment, getPatient } from "@/utils/patientManagement";
  ...
  const updateTreatmentData = (sectionName, fieldKey, value) => {
	if (selectedTemplate.value.fields[sectionName][fieldKey].type === "Boolean") {
	  treatmentData.value[sectionName][fieldKey] = value === "true";
	} else {
	  treatmentData.value[sectionName][fieldKey] = value;
	}
  };
  ...
  const groupedTreatments = computed(() => {
	if (
	  !patientData.value ||
	  !patientData.value.treatments ||
	  patientData.value.treatments.length === 0
	)
	  return {};
	return patientData.value.treatments.reduce((acc, treatment) => {
	  const profession = treatment.template?.profession || "Unknown";
	  if (!acc[profession]) acc[profession] = [];
	  acc[profession].push(treatment);
	  return acc;
	}, {});
  });
  ...
  const fetchTreatmentTemplates = async () => {
	try {
	  ...
	  treatmentTemplates.value = response.data.treatmentTemplates;
	} catch (error) {
	  console.error("Error fetching treatment templates:", error);
	  if (error.response && error.response.status === 404) {
		toast.warning(
		  "No treatment templates found for your profession. Please contact an administrator."
		);
		treatmentTemplates.value = [];
	  } else {
		toast.error("Failed to fetch treatment templates. Please try again.");
	  }
	} finally {
	  loadingModal.value.hide();
	}
  ...
  const selectTemplate = (template) => {
	...
	treatmentData.value = Object.keys(template.fields).reduce(
	  (acc, sectionName) => {
	...
	"Initial Treatment Data:", treatmentData.value; // For debugging
  };
  
  const resetForm = () => {
	if (selectedTemplate.value) {
	  treatmentData.value = Object.keys(selectedTemplate.value.fields).reduce(
		(acc, sectionName) => {
  ...
  const handleImageUpload = async (event, sectionName, fieldKey) => {
	...
	loadingModal.value.show();
	try {
	  const downloadURL = await uploadImage(file, "Treatments");
	  if (downloadURL) {
		updateTreatmentData(sectionName, fieldKey, downloadURL);
  ...
  const deleteImage = async (sectionName, fieldKey) => {
	const imageUrl = treatmentData.value[sectionName][fieldKey];
	if (!imageUrl) return;
  
	loadingModal.value.show();
	try {
	  await deleteFirebaseImage(imageUrl);
	  updateTreatmentData(sectionName, fieldKey, "");
  ...
  const submitTreatment = async () => {
	try {
	  ...
	  const transformedTreatmentData = Object.entries(
		treatmentData.value
	  ...
	  "Transformed Treatment Data:", transformedTreatmentData;
  
	  const response = await createTreatment(
		selectedTemplate.value._id,
		transformedTreatmentData,
	  ...
	  toast.error(`Failed to submit treatment: ${error.message}`);
	} finally {
	  loadingModal.value.hide();
	}
  };
