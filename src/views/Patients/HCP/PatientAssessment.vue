                ...
                >
                  ...
                  <template v-if="field.isImage">
                    <div class="flex items-center space-x-4">
                      ...
                      <span
                        v-if="assessmentData[sectionName][fieldKey]"
                    ...
                    >
                      <img
                        :src="assessmentData[sectionName][fieldKey]"
                        alt="Uploaded image"
                  ...
                  >
                    ...
                    >
                      <h3 class="text-xl font-semibold">
                        {{ assessment.template?.name || "Unnamed Assessment" }}
                      </h3>
...
const updateAssessmentData = (sectionName, fieldKey, value) => {
  if (selectedTemplate.value.fields[sectionName][fieldKey].type === "Boolean") {
    assessmentData.value[sectionName][fieldKey] = value === "true";
  } else {
...
const selectTemplate = (template) => {
  ...
  "Initial Assessment Data:", assessmentData.value; // For debugging
};
...
const handleImageUpload = async (event, sectionName, fieldKey) => {
  ...
  loadingModal.value.show();
  try {
    const downloadURL = await uploadImage(file, "Assessments");
    if (downloadURL) {
      updateAssessmentData(sectionName, fieldKey, downloadURL);
...
const deleteImage = async (sectionName, fieldKey) => {
  const imageUrl = assessmentData.value[sectionName][fieldKey];
  if (!imageUrl) return;

  loadingModal.value.show();
  try {
    await deleteFirebaseImage(imageUrl);
    updateAssessmentData(sectionName, fieldKey, "");
...
const submitAssessment = async () => {
  try {
    ...
    "Transformed Assessment Data:", transformedAssessmentData;
