<template>
    <div class="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div class="max-w-7xl mx-auto">
        ...
        <div v-if="activeTab === 'new' && patientData">
          <div v-if="referralTemplates.length === 0" class="bg-white shadow-lg rounded-lg overflow-hidden p-6">
            <p class="text-lg text-gray-600">No referral templates are available for your profession.</p>
            <p class="text-md text-gray-500 mt-2">Please contact an administrator to create templates before adding new referrals.</p>
          </div>
          <!-- Template Selection -->
          <div
          ...
          >
            ...
            <div class="p-6">
              <div v-if="referralTemplates.length === 0" class="text-center">
                <p class="text-lg text-gray-600">No referral templates available for your profession.</p>
                ...
                >
                  ...
                  >
                    ...
                    <component
                      ...
                      @input="updateReferralData(sectionName, fieldKey, $event.target.value)"
        ...
        <div v-else-if="activeTab === 'past' && patientData" class="space-y-8">
          <div v-if="Object.keys(groupedReferrals).length === 0" class="bg-white shadow-lg rounded-lg overflow-hidden p-6">
            <p class="text-lg text-gray-600">No past referrals found for this patient.</p>
          </div>
          <div
            v-else
  ...
  * {
  ...
  import { getReferralTemplatesByProfession } from "@/utils/referralTemplate";
  ...
  const groupedReferrals = computed(() => {
    if (!patientData.value || !patientData.value.referrals || patientData.value.referrals.length === 0) return {};
  ...
  const fetchReferralTemplates = async () => {
    try {
      loadingModal.value.show();
      const response = await getReferralTemplatesByProfession(
      ...
      if (error.response && error.response.status === 404) {
        toast.warning("No referral templates found for your profession. Please contact an administrator.");
  ...
  const selectTemplate = (template) => {
    ...
    referralData.value = Object.keys(template.fields).reduce((acc, sectionName) => {
  ...
  const resetForm = () => {
    if (selectedTemplate.value) {
      referralData.value = Object.keys(selectedTemplate.value.fields).reduce((acc, sectionName) => {
  ...
  const submitReferral = async () => {
    try {
      ...
      // Transform the referralData to use labels instead of field keys
      const transformedReferralData = Object.entries(referralData.value).reduce((acc, [sectionName, sectionData]) => {
      ...
      console.log('Transformed Referral Data:', transformedReferralData); // For debugging
      ...
      if (response.data && response.data.status === "Success") {
        ...
        resetForm();
        await fetchPatientData(); // Refresh patient data to include the new referral
        activeTab.value = 'past'; // Switch to past referrals tab
      } else {
