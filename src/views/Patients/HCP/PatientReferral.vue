			...
			<div v-if="activeTab === 'new' && patientData">
				...
				>
					...
					<div class="p-6">
						...
						>
							<p class="text-lg text-gray-600">
								No referral templates available for your
								profession.
							</p>
							<p class="text-md text-gray-500 mt-2">
							...
							>
								...
								>
									...
									<component
										...
										:id="`${sectionName}_${fieldKey}`"
										:value="
											referralData[sectionName][fieldKey]
										"
										@input="
											updateReferralData(
												sectionName,
...
<script setup>
	...
	const fetchReferralTemplates = async () => {
		try {
			...
			if (error.response && error.response.status === 404) {
				...
				toast.error(
					'Failed to fetch referral templates. Please try again.'
				);
			}
		} finally {
			loadingModal.value.hide();
	...
	const resetForm = () => {
		if (selectedTemplate.value) {
			referralData.value = Object.keys(
