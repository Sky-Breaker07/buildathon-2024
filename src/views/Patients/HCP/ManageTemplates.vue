<template>
	...
	>
		...
		<div
			...
			class="text-center text-gray-600 text-xl mt-12"
		>
			...
			>
				<div class="p-6">
					...
					<div class="flex justify-between items-center">
						...
						<button
							@click="showDeleteModalFn(template)"
				...
				>
					...
					<div class="max-h-[70vh] overflow-y-auto">
						...
						>
							...
							<div
								v-for="(field, fieldName) in section"
								:key="fieldName"
								class="mb-4"
							>
...
<script setup>
	...
	const fetchTemplates = async () => {
		try {
			...
			'API Response:', response; // For debugging
			...
			if (error.response && error.response.status === 404) {
				toast.info(
					'No templates found for your profession. Please create templates.'
				);
			} else {
	...
	const editTemplate = (template) => {
		router.push({
			name: 'EditTemplates',
			params: { type: templateType.value, id: template._id },
		});
	};
