						...
						<td class="py-4 px-2 border-b">
							...
							<template v-if="patient.status === 'pending'">
								<button
									...
									"
									class="bg-green-500 text-white px-2 py-1 rounded hover:bg-green-600 mr-4"
								>
									Accept
								</button>
								<button
							...
							<template v-else-if="patient.status === 'accepted'">
								<button
									...
									"
									class="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600 mr-4"
								>
									Assign
								</button>
								<button
			...
			<div class="mb-4">
				...
				>
					<input
						type="checkbox"
						id="selectAll"
						v-model="selectAllHCPs"
						@change="toggleAllHCPs"
						class="mr-2"
					/>
					Select All HCPs
				</label>
...
<script setup>
	...
	const fetchHCPs = async () => {
		try {
			...
			if (
				...
				toast.error(
					'Failed to fetch healthcare professionals. Unexpected data structure.'
				);
			}
		} catch (error) {
	...
	const filterPatients = (patientList) => {
		return patientList.filter((patient) => {
			...
			const matchesFilter =
				...
				(filterOption.value === 'otherHCPs' &&
					patientList !== patientStore.allPatients.assignedToAdmin &&
					patientList !==
						patientStore.allPatients.receivedPatients) ||
	...
	const confirmAssignment = async () => {
		if (selectedHCPs.value.length === 0) {
			toast.error(
				'Please select at least one HCP to assign the patient to.'
			);
			return;
		}
		...
		try {
			...
			patientStore.removePatientFromReceived(
				selectedPatient.value.hospital_id
			);
