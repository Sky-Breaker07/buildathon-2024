<template>
	...
	<div class="w-[90rem] mx-auto px-[2.175rem] py-8">
		...
		<div
			v-if="filteredAssignedToAdmin.length > 0"
			class="mb-8"
		>
				...
				<tbody>
					...
					>
						...
						<td class="p-2 border-b space-x-2">
							...
							<button
								@click="
									handleUnassignPatient(patient.hospital_id)
								"
		...
		<div
			v-if="filteredAssignedToOtherHCPs.length > 0"
			class="mt-10 mb-8"
		>
			<h2 class="text-2xl font-poppins font-semibold mb-4">
				Patients Assigned to Other HCPs
		...
		<!-- Received Patients -->
		<div
			v-if="filteredReceivedPatients.length > 0"
			class="mb-8 font-poppins"
		>
					...
					>
						...
						<td class="py-4 px-2 border-b space-x-2">
							...
							<template v-if="patient.status === 'pending'">
								<button
									@click="
										handleAcceptPatient(patient.hospital_id)
									"
								...
								<button
									@click="
										handleRejectPatient(patient.hospital_id)
									"
							...
							<template v-else-if="patient.status === 'accepted'">
								<button
									@click="
										handleAssignPatient(patient.hospital_id)
									"
	...
	<div
		...
		class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex justify-center items-center font-poppins"
	>
		<div class="bg-white p-5 rounded-lg shadow-xl w-full max-w-md">
			...
			<div class="flex justify-end">
				...
				<button
					...
					class="bg-blue-500 hover:bg-blue-700 text-white font-meduim py-2 px-4 rounded"
				>
					Assign
				</button>
			</div>
...
<script setup>
	...
	const fetchHCPs = async () => {
		try {
			...
			if (
				...
				availableHCPs.value =
					response.data.data.healthcareProfessionals.map((hcp) => ({
			...
			toast.error(
				'Failed to fetch healthcare professionals. Please try again.'
			);
		} finally {
			loadingModal.value.hide();
		}
	...
	const filteredReceivedPatients = computed(() => {
		const assignedPatientIds = [
			...
			...filteredAssignedToOtherHCPs.value.flatMap((group) =>
				group.patients.map((p) => p.hospital_id)
			),
		];

		return filterPatients(
			patientStore.allPatients.receivedPatients.filter(
				(p) => !assignedPatientIds.includes(p.hospital_id)
			)
		);
	});
			...
			const matchesFilter =
				...
				(filterOption.value === 'assigned' &&
					(patientList === patientStore.allPatients.assignedToAdmin ||
						patientStore.allPatients.assignedToOtherHCPs.some(
							(group) => group.patients.includes(patient)
						))) ||
	...
	const handleAcceptPatient = async (hospitalId) => {
		try {
			...
			await acceptPatient(
				hospitalId,
				staffStore.currentUser.staff_id,
				token
			);
